...
public final class OdinSdksConfigurable implements Configurable {
    ...
    private SimpleColoredComponent version;
    ...
    public @Nullable JComponent createComponent() {
        ...
        list.addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting() && !updating) select(list.getSelectedIndex());
        });
        ...
        compiler.getTextField().getDocument().addDocumentListener(new DocumentAdapter() {
            @Override
        ...
        version = new SimpleColoredComponent();
        ...
        component = new JPanel(new BorderLayout(0, 12));
        component.add(splitter, BorderLayout.CENTER);
        component.add(check, BorderLayout.SOUTH);
        load();
        return component;
    }
    ...
    private JComponent createEditor() {
        ...
        addRow(panel, 0, "Name:", name);
        addRow(panel, 1, "Odin executable:", compiler);
        addRow(panel, 2, "Odin libraries:", libraries);
        addRow(panel, 3, "Version:", version);
    ...
    private static void addRow(JPanel panel, int row, String text, JComponent field) {
        ...
        label.insets = JBUI.insets(0, 0, 8, 8);
        ...
        value.insets = JBUI.insetsBottom(8);
    ...
    private void addLocal() {
        ...
        value.name = dialog.sdkName();
        value.compilerPath = dialog.compilerPath();
        value.libraryPath = dialog.libraryPath();
        value.origin = "LOCAL";
        if (new File(value.compilerPath).isFile())
            value.version = OdinSdkUtils.getOdinCompilerVersion(value.compilerPath);
        addToList(value);
    }

    private void download() {
        final List<OdinRelease>[] releases = new List[]{List.of()};
        final Exception[] failure = new Exception[1];
        ProgressManager.getInstance().runProcessWithProgressSynchronously(() -> {
            try {
                releases[0] = OdinReleaseService.fetchReleases(true).stream().limit(20).toList();
            } catch (Exception e) {
                failure[0] = e;
            }
        }, "Checking for Odin Releases", true, null);
        if (failure[0] != null) {
            showError("Unable to retrieve Odin releases: " + failure[0].getMessage());
            return;
        }
        if (releases[0].isEmpty()) {
            showError("GitHub did not return any downloadable Odin releases.");
            return;
        }
    ...
    private void addToList(OdinSdkState value) {
        flush();
        items.add(value);
        sdkListPanel.addSdk(value);
    }

    private void showError(String text) {
        JOptionPane.showMessageDialog(component, text, "Odin SDK", JOptionPane.ERROR_MESSAGE);
    }

    private void select(int index) {
        flush();
        selected = index;
        OdinSdkState value = index >= 0 && index < items.size() ? items.get(index) : null;
        updating = true;
        name.setText(value == null ? "" : value.name);
        compiler.setText(value == null ? "" : compilerPath(value));
        libraries.setText(value == null ? "" : libraryPath(value));
        name.setEnabled(value != null);
        compiler.setEnabled(value != null);
        libraries.setEnabled(value != null);
        updating = false;
    ...
    private void checkCompilerVersion() {
        ...
        String compilerPath = compiler.getText();
        if (compilerPath == null || compilerPath.isBlank()) return;
        if (!new File(compilerPath).isFile()) {
            version.setIcon(AllIcons.General.Error);
            version.append("Invalid executable path");
            return;
        }
        version.setIcon(AnimatedIcon.Default.INSTANCE);
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            String detected = OdinSdkUtils.getOdinCompilerVersion(compilerPath);
            if (request != versionRequest || component == null || !compilerPath.equals(compiler.getText())) return;
            version.clear();
            if ("Unknown".equals(detected)) {
                OdinSdkState sdk = selected >= 0 && selected < items.size() ? items.get(selected) : null;
                if (sdk != null && sdk.releaseTag != null && !sdk.releaseTag.isBlank()) {
                    version.append(sdk.releaseTag);
                } else {
                    version.setIcon(AllIcons.General.Error);
                    version.append("Unable to determine Odin version");
                }
            } else {
                version.setIcon(AllIcons.General.GreenCheckmark);
                version.append(detected);
                if (selected >= 0 && selected < items.size()) items.get(selected).version = detected;
            }
        });
    }
    ...
    private void flush() {
        ...
        OdinSdkState value = items.get(selected);
        value.name = name.getText();
        value.compilerPath = compiler.getText();
        value.libraryPath = libraries.getText();
        value.homePath = "";
        list.repaint();
    }
    ...
    private void remove() {
        ...
        if (i < 0) return;
        OdinSdkState value = items.get(i);
        ...
        selected = -1;
        items.remove(i);
        sdkListPanel.removeAt(i);
        if (items.isEmpty()) select(-1);
        else list.setSelectedIndex(Math.min(i, items.size() - 1));
    }
    ...
    public void apply() throws ConfigurationException {
        ...
        for (OdinSdkState value : items) {
            if (value.name == null || value.name.isBlank())
                throw new ConfigurationException("SDK name must not be empty");
            if (!new File(value.compilerPath).isFile())
                throw new ConfigurationException("Odin executable does not exist: " + value.compilerPath);
            if (!new File(value.libraryPath).isDirectory())
                throw new ConfigurationException("Odin library directory does not exist: " + value.libraryPath);
        }
    ...
    private void load() {
        ...
        sdkListPanel.setSdks(items);
        check.setSelected(OdinSdkRegistryService.getInstance().getState().automaticallyCheckForReleases);
        updating = false;
        if (items.isEmpty()) select(-1);
        else list.setSelectedIndex(0);
    }

    private static String compilerPath(OdinSdkState s) {
        return s.compilerPath == null || s.compilerPath.isBlank() ? safeLegacyCompiler(s.homePath) : s.compilerPath;
    }

    private static String libraryPath(OdinSdkState s) {
        return s.libraryPath == null || s.libraryPath.isBlank() ? Objects.requireNonNullElse(s.homePath, "") : s.libraryPath;
    }

    private static String safeLegacyCompiler(String home) {
        return home == null || home.isBlank() ? "" : OdinSdkUtils.getOdinBinaryPath(home);
    }

    private static OdinSdkState copy(OdinSdkState s) {
        OdinSdkState v = new OdinSdkState();
        v.id = s.id;
        v.name = s.name;
        v.compilerPath = compilerPath(s);
        v.libraryPath = libraryPath(s);
        v.version = s.version;
        v.origin = s.origin;
        v.releaseTag = s.releaseTag;
        v.releaseId = s.releaseId;
        v.releaseAssetId = s.releaseAssetId;
        v.releaseAssetUrl = s.releaseAssetUrl;
        return v;
    }
    ...
    private final class SdkListPanel extends AddDeleteListPanel<OdinSdkState> {
        private SdkListPanel() {
            super(null, List.of());
        }

        @Override
        protected @Nullable OdinSdkState findItemToAdd() {
            return null;
        }
        ...
        @Override
        protected ListCellRenderer<? super OdinSdkState> getListCellRenderer() {
        ...
        private JBList<OdinSdkState> sdkList() {
            return myList;
        }

        private void addSdk(OdinSdkState sdk) {
            addElement(sdk);
        }
        ...
        private void setSdks(List<OdinSdkState> sdks) {
            myListModel.clear();
            for (OdinSdkState sdk : sdks) myListModel.addElement(sdk);
        }
    }
}
